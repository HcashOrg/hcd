...
func ExtractP2XScriptSigType(sdb ScriptDB, chainParams *chaincfg.Params, pkScript []byte) ([]uint8, error) {
	class, addresses, required, err := ExtractPkScriptAddrs(DefaultScriptVersion, pkScript, chainParams)
	if err != nil {
		return []uint8{uint8(chainec.ECTypeSecp256k1)}, err
	}

	switch class {
	case PubKeyTy:
		return []uint8{uint8(chainec.ECTypeSecp256k1)}, nil
	case PubkeyAltTy:
		sigType , err :=  ExtractPkScriptAltSigType(pkScript)
		return []uint8{sigType}, err
	case PubKeyHashTy:
		return []uint8{uint8(chainec.ECTypeSecp256k1)}, nil
	case PubkeyHashAltTy:
		sigType , err :=  ExtractPkScriptAltSigType(pkScript)
	...
	case MultiSigTy:
		...
		for _, addr := range addresses {
			switch addr.(type) {
			case *hcutil.AddressSecpPubKey:
				sigTypes = append(sigTypes, chainec.ECTypeSecp256k1)
			case *hcutil.AddressBlissPubKey:
		...
		sort.Sort(sort.Reverse(sort.IntSlice(sigTypes)))
		var uSigType []uint8
		for i :=0; i<required; i++ {
			uSigType = append(uSigType, uint8(sigTypes[i]))
		}
		return uSigType, nil
	case StakeSubmissionTy:
		return []uint8{uint8(chainec.ECTypeSecp256k1)}, nil
