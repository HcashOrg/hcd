...
func newServer(listenAddrs []string, witnessListenAddrs []string, db database.DB, chainParams *chaincfg.Params) (*server, error) {
	...
	s := server{
		...
		newWitnessPeers:             make(chan *serverWitnessPeer, cfg.MaxPeers),
		doneWitnessPeers:            make(chan *serverWitnessPeer, cfg.MaxPeers),
		banWitnessPeers:             make(chan *serverWitnessPeer, cfg.MaxPeers),
		...
		modifyRebroadcastWitnessInv: make(chan interface{}),
